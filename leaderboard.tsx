...
function Leaderboard() {
  ...
  const rows = data?.topBalance ?? [];

  return (
    <div className="space-y-4 p-4 pt-6">
      <h1 className="font-display text-3xl text-gradient-gold">Leaderboard</h1>

      <GoldFrame className="flex items-center justify-center gap-2 p-3 text-gold-soft">
        <Trophy className="h-4 w-4" />
        <span className="font-display text-xs font-semibold uppercase tracking-widest">Top Balance</span>
      </GoldFrame>
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {rows.map((r, idx) => (
            <Row key={r.telegram_id} row={r} rank={idx + 1} unit="GTC" />
          ))}
        </div>
      )}
    </div>
