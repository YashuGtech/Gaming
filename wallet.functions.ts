...
const SubmitDepositInput = z.object({
  ...
  declaredAmountUsdt: z.number().positive().max(1_000_000),
...
export const submitDeposit = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const amountGtc = data.declaredAmountUsdt / rate;

    const { data: inserted, error: insertErr } = await supabaseAdmin
      .from("deposits")
      .insert({
        user_id: user.telegram_id,
        amount_usdt: data.declaredAmountUsdt,
    ...
    if (token && inserted) {
      ...
      const msg =
        ...
        `Amount: <b>${data.declaredAmountUsdt} USDT</b> → ${amountGtc.toFixed(2)} GTC\n` +
        `TX: <code>${txHash}</code>`;
      await Promise.all(
